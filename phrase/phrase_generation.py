...
def generate_phrase_dictionary(corpus_func, min_word_count=1, max_phrase_count=2, colloc_rounds=4):
    """
    ...
    :return:
    """
    max_phrases_per_round = math.floor(max_phrase_count/colloc_rounds)
    phrase_dictionary = PhraseDictionary()
    for i in range(colloc_rounds):
        phrase_dictionary = extend_phrase_dictionary(corpus_func(),
                                PhraseDictionary.generate_phrase_detection_function(min_token_count=min_word_count, max_phrases=max_phrases_per_round),
                                phrase_dictionary)
    return phrase_dictionary
